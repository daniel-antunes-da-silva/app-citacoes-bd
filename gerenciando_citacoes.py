    ...
    while True:
        ...
        if escolha_do_usuario == '1':
            ...
            while True:
                ...
                if confirm != 'n':
                    cursor.execute('''
                        INSERT INTO citacoes(citacao, autor)
                        values(:citacao, :autor)
                        ''', dados)
                    print('Citação inserida com sucesso!')
                    cursor.execute('''UPDATE citacoes
                                   SET id_citacao = (SELECT COUNT(*) FROM citacoes c
                                   WHERE c.id_citacao < citacoes.id_citacao) + 1''')
                else:
                    conexao.commit()
            ...
            while True:
                ...
                try:
                    if escolha_alteracao == '1':
                        ...
                        dados['autor'] = str(input('Digite o novo autor: ')).strip().title()
                        cursor.execute('''
                        UPDATE citacoes
                        SET autor = ?
            ...
            for result in resultado:
                for r in range(0, 3):
                    print(f'{result[r]:<2}  ', end=f'{simbolos[r]}  ')
                print()
